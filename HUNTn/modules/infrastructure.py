...
Covers: naabu port scanning, nmap service detection, TLS/SSL (tlsx, sslscan),
        VHost discovery, ASN IP space, Shodan CLI
"""
...
def run(ctx):
    ...
    if which("nmap"):
        # Extract IPs from port scan results
        run_cmd_pipe(
            f"cat {ports_file} 2>/dev/null | grep -oE '([0-9]{{1,3}}\\.?){{4}}' | sort -u",
            output_file=str(infra_dir / "ips.txt")
        )
    ...
    if which("tlsx") and live_file.exists():
        run_cmd_pipe(
        ...
        # Extract SANs (Subject Alternative Names) as extra subdomain source
        san_file = ws.path("subdomains", "tls_san.txt")
        run_cmd_pipe(
            f"cat {tls_file} | python3 -c \"import sys,json; [print(s) for l in sys.stdin "
            f"for d in [json.loads(l) if l.strip() else {{}}] for s in d.get('san',[]) if s]\" 2>/dev/null",
            output_file=str(san_file)
        )
        print_success("SAN domains extracted → subdomains/tls_san.txt")
    else:
    ...
    vhost_wl = config.get("wordlists", {}).get("vhosts",
        "/usr/share/seclists/Discovery/DNS/subdomains-top1million-50000.txt")

    if which("ffuf") and Path(vhost_wl).exists():
        # Get target IP for VHost bruteforce
        import subprocess
