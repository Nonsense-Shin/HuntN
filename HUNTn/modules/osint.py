...
Covers: WHOIS, DNS, ASN, crt.sh, OTX, VirusTotal, URLScan, Shodan, GitHub dorking hints
"""
...
def run(ctx):
    ...
    if which("whois"):
        run_cmd(["whois", target],
                output_file=ws.path("intelligence", "whois.txt"))
        print_success("whois.txt saved")
    else:
        print_skip("whois not found — sudo apt install whois")

    # ── 1.2 DNS RECORDS ────────────────────────────────────────
    print_info("DNS records (A, MX, NS, TXT, CNAME, SOA)...")
    ...
    # ── 1.5 SUBDOMAINS FROM INTELLIGENCE SOURCES ───────────────
    print_info("AlienVault OTX passive lookup...")
    otx_file = ws.path("intelligence", "otx.txt")
    _otx_lookup(target, otx_file, apis.get("otx", ""))

    print_info("URLScan.io lookup...")
    urlscan_file = ws.path("intelligence", "urlscan.txt")
    _urlscan_lookup(target, urlscan_file, apis.get("urlscan", ""))

    print_info("VirusTotal passive lookup...")
    vt_file = ws.path("intelligence", "virustotal.txt")
    _virustotal_lookup(target, vt_file, apis.get("virustotal", ""))

    print_info("Wayback Machine URL collection...")
    wayback_file = ws.path("intelligence", "wayback_domains.txt")
    _wayback_domains(target, wayback_file)

    # ── 1.6 SHODAN HINTS ────────────────────────────────────────
    _shodan_info(target, ws, apis.get("shodan", ""))

    # ── 1.7 MANUAL OSINT CHECKLIST ──────────────────────────────
    _generate_osint_checklist(target, ws)

    # ── 1.8 GOOGLE DORKS ────────────────────────────────────────
    _generate_google_dorks(target, ws)

    # ── 1.9 GIT DORKING HINTS ───────────────────────────────────
...
def _check_email_security(target, ws):
    ...
    with open(report_path, "w") as f:
        ...
        f.write("\n\n## DMARC\n")
        f.write(dmarc.stdout.strip() if dmarc.stdout.strip() else "NO DMARC RECORD FOUND — Potential email spoofing!\n")

    if not spf_records:
        print_find("No SPF record → potential email spoofing target!")
    if not dmarc.stdout.strip():
...
def _otx_lookup(target, output_file, api_key=""):
    ...
    try:
        ...
        urls = [entry.get("url", "") for entry in data.get("url_list", []) if entry.get("url")]
        with open(output_file, "w") as f:
            f.write("\n".join(urls))
...
def _wayback_domains(target, output_file):
    """Collect unique domains from Wayback Machine CDX API."""
    try:
        url = f"http://web.archive.org/cdx/search/cdx?url=*.{target}&fl=original&collapse=urlkey&output=text&limit=2000"
...
def _shodan_info(target, ws, api_key=""):
    """Provide Shodan recon instructions and run asnmap if available."""
...
def _generate_github_dorks(target, ws):
    ...
    with open(github_file, "w") as f:
        ...
        # Strip TLD for org name guessing
        org = target.split(".")[0]
